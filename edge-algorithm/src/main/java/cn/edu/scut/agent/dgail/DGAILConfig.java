...
public class DGAILConfig implements InitializingBean {
    @Value("${rl.epoch}")
    private int epoch;
    ...
    @Bean
    public Model diffusionModel(NDManager manager) {
        var model = Model.newInstance("diffusion");
        model.setBlock(createDiffusionNetwork(manager));
        return model;
    }

    private NDArray swish(NDArray x) {
        return x.mul(Activation.sigmoid(x));
    }

    private Block createDiffusionNetwork(NDManager manager) {
        var block = new SequentialBlock();
        int combinedDim = observationShape + actionShape;
        
        block.add(Linear.builder().setUnits(hiddenShape).build());
        block.add(arrays -> new NDList(swish(arrays.singletonOrThrow())));
        
        block.add(Linear.builder().setUnits(hiddenShape * 2).build());
        block.add(arrays -> new NDList(swish(arrays.singletonOrThrow())));
        block.add(Linear.builder().setUnits(hiddenShape * 2).build());
        block.add(arrays -> new NDList(swish(arrays.singletonOrThrow())));
        block.add(Linear.builder().setUnits(combinedDim).build());
        
        block.initialize(manager, DataType.FLOAT32,
            new Shape(batchSize, combinedDim + 1)); // +1 for timestep
        return block;
    }

    @Bean
    public TrainingConfig diffusionTrainingConfig(Optimizer optimizer) {
