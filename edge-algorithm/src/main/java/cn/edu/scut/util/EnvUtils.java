...
public class EnvUtils implements InitializingBean {
    ...
    public boolean isMeetReliabilityRequirement(Set<Integer> set, Task task) {
        ...
        log.info("task id:{}, task job_id:{} source: {}, set:{}" ,task.getId() ,task.getJobId(), task.getSource(), set);
        for (Integer s : set) {
            if(s == offloadShape){
                continue;
            }
            var taskReliability = task.getTaskReliability();
            log.info("task job_id:{} source: {}, destination: {}",task.getJobId(), task.getSource(), s);
