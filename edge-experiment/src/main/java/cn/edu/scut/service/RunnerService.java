...
public class RunnerService implements InitializingBean {
    ...
    public void run() {
        ...
        for(int i=0;i<edgeNodeNumber;i++){
            List<Integer> a = restTemplate.getForObject("http://edge-node-"+(i+1)+"/user/result", List.class);
            result.add(a);
        }
        transitionService.setResult(result);
    ...
    public void updateModel(String flag) {
        ...
        transitionService.setResult(new ArrayList<>());
    }

    
    public void addData() {
        var teamRewards = new float[timeSlotNumber * 100];
        var states = new float[timeSlotNumber][edgeNodeNumber][stateShape];
        var actions = new int[timeSlotNumber][edgeNodeNumber][1];
        var passiveRedundancyActions = new int[timeSlotNumber][edgeNodeNumber][1];
            ...
            for (int j = 1; j <= edgeNodeNumber; j++) {
                ...
                if (useRewardInTime) {
                    ...
                    if (task != null) {
                        ...
                        var endTimeSlot = task.getTimeSlot() + totalTime / timeSlotLen; // 任务的起始timeSlot加上（任务的总时间除以timeSlot的长度）
